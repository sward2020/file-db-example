package com.lmig.ci.lmb.gi.config;


import com.lmig.ci.lmb.gi.dao.SettlementFileRepositoryImpl;
import com.lmig.ci.lmb.gi.dao.entities.MopCodeRef;
import com.lmig.ci.lmb.gi.dao.entities.SettlementFile;
import com.lmig.ci.lmb.gi.interfaces.MopCodeRepository;
import com.lmig.ci.lmb.gi.interfaces.SettlementFileRepository;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.boot.autoconfigure.EnableAutoConfiguration;
import org.springframework.boot.autoconfigure.data.redis.RedisAutoConfiguration;
import org.springframework.boot.autoconfigure.domain.EntityScan;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.ComponentScan;
import org.springframework.context.annotation.Configuration;

import org.springframework.data.jpa.repository.config.EnableJpaRepositories;

/**
 * Created by n0050370 on 5/24/17.
 */

@Configuration
@EntityScan(basePackageClasses= {MopCodeRef.class})
@ComponentScan(basePackageClasses = {SettlementFileRepositoryImpl.class})
@EnableJpaRepositories(basePackages = {"com.lmig.ci.lmb.gi.interfaces"})
public class ApplicationConfig {
private static final Logger logger = LoggerFactory.getLogger(ApplicationConfig.class);

}
